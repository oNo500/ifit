...
- ORM 默认 Drizzle——SQL 透明可见（区别于 Prisma 的黑箱抽象），edge 运行时必选；使用与迁移纪律见 orm-ts rule。
- lint/format 用 oxlint + oxfmt，不用 ESLint + Prettier 组合。
- type check 用 `tsc --noEmit`。
- 架构边界（feature 隔离、依赖单向、循环依赖检测）用 dependency-cruiser。
