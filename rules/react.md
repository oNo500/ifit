...
- 组件用函数声明 + Props 类型注解，MUST NOT 用 `React.FC`——其隐式 children 行为随版本变动，且写不了泛型组件
...
- Props 容器首选 `interface`（可 extends、声明合并）；联合、交叉、映射类型才用 `type`
  - 正：`interface UserCardProps { user: User }`；反：`type UserCardProps = { user: User }`
- Context MUST 用 `createContext<T | null>(null)` + 守卫 hook——默认值兜底会让缺 Provider 的组件静默拿到假数据，`createContext<T>(null!)` 断言把错误推迟到运行时解引用处才崩；守卫 hook 把接线错误在最近处抛出
