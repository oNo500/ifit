...
thinking token 计为 output 的黑洞与 llm-cost 的降本全景是同一件事的两半，推理模型的选型与 effort 调参归 `reasoning-models` skill，两边互留一行指针。
...
压缩先做朴素 baseline，常常足够：删空行与重复内容、旧轮次摘要替换、few-shot 示例从 5 个降到 2 个、文档先结构化抽取再喂大模型。声称通用有效的优化手段（如 LLMLingua 类工具）MUST 先在自己的业务任务上 eval 通过才上线，论文或厂商的综合分不作数——同一手段在不同任务类型上的实际效果差异极大（摘要 / Q&A 友好，数学 / 代码慎用）。先做朴素 baseline，它常常已经够用；baseline 打不过再上复杂方案。
