...
describe('showReport', () => {
  ...
  test('catReport returns artifact content verbatim; unknown rule exits 1', async () => {
    ...
    const miss = await catReport(fakeCtx(), { source, name: 'nope' })
    expect(miss.ok).toBe(false)
    expect(miss.exitCode).toBe(1)
  })

  test('missing catalog and source resolution failure both fail cleanly with exitCode 1', async () => {
    const uninitTarget = mkdtempSync(join(tmpdir(), 'iuse-show-uninit-'))
    ...
    const badSourceResult = await showReport(fakeCtx(), { source: badSource, target: uninitTarget, name: 'alpha' })
