...
├── globals.json               # 全局层账：~/.claude 应装 rule 清单（ifit --global 只读对账）
...
- 本仓不产资产：skills/、rules/、templates/ 与四账（catalog/profiles/
  globals/skills）的内容一律来自开发仓 `~/code/infra-agent/iforge` 的 `iforge publish`
...
- 使用端口径，面向下游项目和设备：
  - `ifit status [--global]` — 逐 rule 对账下游副本与本仓 catalog 的
