...
`UserRepository` 接口放 `application/ports/`，`DrizzleUserRepository` 在 `infrastructure/` 实现，数据库客户端只注入到这一层，业务层对具体 ORM 无感知。
...
NestJS DI 依赖 constructor 参数的运行时类引用，两处与常规 TS 约定冲突，MUST 按例外处理：

- MUST 关闭 `typescript/consistent-type-imports` lint 规则——该规则会把注入类错误转成 `import type`，类型擦除后 DI token 变 undefined，注入失败
