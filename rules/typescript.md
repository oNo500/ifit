...
TS 的类型红线与日常纪律，只覆盖写 TS 时无法从代码推断的项目偏好与 lint 管不到的判断（断言边界、收窄方式、`enum` 取舍）。

## 权威校验

- 编辑批次完成后跑 `tsc --noEmit` 与 oxlint，据其输出判断改动是否成立（对应 agent-behavior 「不响应实时 LSP 诊断」在 TS 侧的命令）

## 红线

- MUST NOT 双重断言——`as X as Y` 经 `unknown`/`any` 中转绕过全部类型检查
  - 反：`value as unknown as Config`；正：修类型建模，或在边界单次 `as`（见「类型断言」）
- MUST NOT 用 `@ts-ignore`/`@ts-expect-error` 或 lint 抑制注释（`eslint-disable`/`oxlint-disable`）——被压掉的错误不会消失，只会失去追踪
...
- 单次 `as` 仅限边界处（`JSON.parse` 结果、外部数据入口），且断言目标写完整结构——内部数据流中需要断言说明类型建模有误，修类型而非加断言
...
- `catch` 到的值类型按 `unknown` 对待；instanceof 收窄或 `String(error)` 转换后再用
  - 正：`catch (error) { log(error instanceof Error ? error.message : String(error)) }`；反：`catch (e) { log(e.message) }`

## 枚举

- MUST NOT 用 `enum`；用 `as const` 对象 + 派生联合类型——字面量联合零运行时开销、无 IIFE 编译产物、类型更安全
  - 正：`const Status = { Open: "open", Done: "done" } as const; type Status = typeof Status[keyof typeof Status]`；反：`enum Status { Open, Done }`

## lint 兜底

- 集合操作用 `toSorted`/`toReversed`、正则带 `u` flag、`catch` 参数命名 `error`——这三条 oxlint 可强制（no-array-sort、require-unicode-regexp、catch-error-name）；配了 lint 以 lint 配置为准，本条只为未配 lint 的项目兜底

## 命名

- 代码内标识符命名（含 `catch` 参数命名、私有字段前缀）见 naming-conventions skill
