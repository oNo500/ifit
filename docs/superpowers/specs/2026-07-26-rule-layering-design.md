...
**已实现(2026-07-26)。** `paths` **只接受数组**,与 Claude Code 的原生
frontmatter 同构——单条也写 `["<glob>"]`。不保留字符串简写:两种写法并存
就要维护归一层,而单一形态让 schema、类型、渲染各处都少一个分支。

改动全在 iforge,四处:

- `assets.schema.json` — `paths` 是 `array(items: string, minItems: 1)`
- `manifest.ts` — `RuleEntry.paths?: string[]`
- `meta.ts` — `MetaAsset.paths: string[] | null`,直接取值不做归一
- `kinds.ts` — 新增导出 `pathsFrontmatter(globs)` 单点定义那个精确字节块,
  `verifyArtifact` 改用它;错误信息 join 全部 glob
- `rule-build.md` — 说明逐条一行,顺序须与 manifest 一致

ifit 侧**无需改动**:它不读 `paths`,产物即安装形态,原样拷贝。

设计决定:空数组由 schema 拒绝——声明了 `paths` 却给零个 glob 是写错了,
静默当 global 会让 rule 无声地从 file-scoped 变成无条件加载。

账已迁移:14 条 file-scoped rule 从 `"**/*.md"` 改为 `["**/*.md"]`,
5 条 global rule 无 `paths` 不变。迁移后 `iforge status` 输出与迁移前一致,
产物字节不变(单条渲染结果相同,故无一条转 outdated)。
测试 152 pass,新增 2 条:多条目渲染含顺序敏感性、单/多/缺省三种形态。
...
- `meta/README.md` 的「元指令文件」节新增「内容边界」小节:判据一句话
  (约束未来的留下、叙述过去的删掉)+ 留删两栏清单 + 混合句改写示例,
  并指明决策依据写 spec 不写元指令
- 六份契约加检查:三份 `*-build.md` 要求读到历史叙述时**报出位置与原文
  由人清理**(不自行改元指令、不写进产物);三份 `*-writeback.md` 从入口
  拦住——回写 MUST NOT 把变更过程写进元指令,产物变了就直接写它现在的约束

选择「报出来由人清理」而非「自动清理」:元指令是意图源,AI 不该自行删改;
而构建器有这个判断力——本轮它主动报了「元指令自身的一处陈述滞后」。
...
机制补强(已做 2026-07-26):三份 `*-build.md` 加「产物已存在时 MUST 只改
元指令变动所涉及的部分,MUST NOT 重写无关段落」,并要求在回复里说明实际改了
哪几处。理由写进契约:全文重写会让人审 diff 淹没在措辞漂移里,已通过审查的
措辞、强度词与列表嵌套结构都可能无声退化。

可行性依据:本轮 visualize 的构建器已自发这么做了——「本次实际修改:把格式
选型里的禁用表格补成…产物其余部分本就与元指令对齐,未作改动」。

注意这条约束**不能消除漂移**,只能缩小它的面。真正的不可重现性来自 AI 生成
本身,人审 diff 仍是唯一防线。
